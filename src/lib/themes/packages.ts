...
function identity(value: unknown): string {
  ...
  if (!/^[a-z0-9][a-z0-9.-]{1,63}$/.test(id) || id === 'constructor' || id === 'prototype') throw new Error('Invalid theme identity.')
  return id
}
export function parseThemePackage(value: unknown): ThemePackage {
  ...
  const id = identity(raw.id), name = text(raw.name, 48), design = record(raw.design)
...
export function parseSharedTheme(value: unknown): ThemePackage {
  ...
  return parseThemePackage({ app: 'izumi', kind: 'theme-package', schemaVersion: 1, themeApi: 1,
    id: `shared.${id.toLowerCase().slice(0, 48)}`, name, author: 'Shared theme', description: 'A personal Theme Studio design. Customize it after installing.', version: '1.0.0', design })
}
