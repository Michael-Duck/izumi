// GENERATED from src/lib/stremio/debrid/providers/torbox.ts by scripts/generate-cloudflare-resolver-core.mjs.
...
async function tb(method: string, path: string, key: string, body?: FormData, priority?: boolean, signal?: AbortSignal): Promise<any> {
...
export const torbox: DebridProvider = {
  ...
  async resolveHash(key, hashOrMagnet, opts) {
    ...
    if (opts?.noAdd) {
      ...
      const list = await tb('GET', '/torrents/mylist?bypass_cache=true', key, undefined, opts?.priority, opts?.signal).catch(() => undefined)
      ...
      const cr = await tb('POST', '/torrents/createtorrent', key, fd, opts?.priority, opts?.signal)
      id = cr.torrent_id
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let files: any[] = []
    await poll(async () => {
      const r = await tb('GET', `/torrents/mylist?bypass_cache=true&id=${id}`, key, undefined, opts?.priority, opts?.signal)
    ...
    const dl = await tb('GET', `/torrents/requestdl?token=${encodeURIComponent(key)}&torrent_id=${id}&file_id=${best.id}`, key, undefined, opts?.priority, opts?.signal)
